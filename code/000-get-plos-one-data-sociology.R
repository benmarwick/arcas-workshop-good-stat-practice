...
#-----------------------------------------------------------------------
# get 1000 sociology articles from PLOS ------------------------------

library(rplos) # ropensci-archive/rplos
library(rvest)

# scale up

# get DOIs for articles
plos_sociology <- searchplos(q='everything:"sociology"', 
...
plos_sociology$data$full_text <- vector("list", length = nrow(plos_sociology_urls))
...
for(i in 1:nrow(plos_sociology_urls)){
  
  plos_sociology$data$full_text[[i]] <- 
    plos_sociology_urls$url[i]  %>% 
...
saveRDS(plos_sociology$data,
        "data/plos_sociology_data.rds")

# get p-values from the full text of these articles, takes a minute
plos_sociology_full_text_ps <- 
  plos_sociology$data %>% 
...
plos_sociology_full_text_ps_test_reporting <- 
  plos_sociology$data %>% 
...
plos_sociology_full_text_ps_clean <- 
  plos_sociology_full_text_ps %>% 
...
plos_sociology_p_values_only <- 
  plos_sociology_full_text_ps_clean 

# save this so we don't have to clean again
write.csv(plos_sociology_p_values_only,
        "data/plos_sociology_p_values_only.csv")
